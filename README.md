...
* [FT] Maneuver node execution telemetry. Orbital data displayed in the info window will update in real time. Maneuver node popup will show decreasing Total Dv value and burn time remaining. Image will update to show the engine on the craft is active (new DB field `CraftImgBurn`). Page will not update to start maneuver, just preload data when maneuver node is detected (already done to show future orbit on map) and execute changes when maneuver starts. Craft icon will stop updating position as it already does. Execution display will be delayed based on the current signal delay calculated for the craft (have popup state waiting for telemetry data). Normal page update after execution when new orbital data period is reached. Need to account for anyone loading page in mid-maneuver. "Telemetry data" is just linear interpolated state change between before/after orbital data. (simple linear interpolation could be problematic for long burns, since with constant thrust speed will increase as fuel weight decreases and thus so too should the data changes. Will re-assess this approach if necessary - possibly include a step modifier based on the before/after total mass value of the vessel). Time to maneuver also has to reflect time to maneuver start, not time to node
